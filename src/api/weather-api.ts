...
async function getCity(city: string) {
    const cityResult = await fetchWithTimeout(
        `${process.env.GEOCODING_URL}?name=${encodeURIComponent(city)}&count=1&language=ru&format=json`
    )
...
export default async function getWeather(cities: string[], days: number) {
    const cords = await getCoords(cities)
    ...
    const data = await Promise.allSettled(
        cords.map(async cord => {
            if (cord.status === 'success') {
                ...
                console.error('Ошибка: не удалось получить координаты города')
            }
        })
    )
